...
func (b *BarChart) Plot(c draw.Canvas, plt *plot.Plot) {
	...
	for i, ht := range b.Values {
		...
		catMin = trCat(float64(catVal)-b.Width/2) + b.Offset
		catMax := trCat(float64(catVal)+b.Width/2) + b.Offset
...
func (b *BarChart) DataRange() (xmin, xmax, ymin, ymax float64) {
	catMin := b.XMin - b.Width/2
	catMax := b.XMin + float64(len(b.Values)-1) + b.Width/2
	...
	for i := range b.Values {
		...
		if !b.Horizontal {
			...
			boxes[i].Rectangle = vg.Rectangle{
				Min: vg.Point{X: b.Offset - font.Length(b.Width/2)},
				Max: vg.Point{X: b.Offset + font.Length(b.Width/2)},
			}
		} else {
			boxes[i].Y = plt.Y.Norm(cat)
			boxes[i].Rectangle = vg.Rectangle{
				Min: vg.Point{Y: b.Offset - font.Length(b.Width/2)},
				Max: vg.Point{Y: b.Offset + font.Length(b.Width/2)},
			}
		}
	}
	return boxes
